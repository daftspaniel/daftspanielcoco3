...
115 A$ = INKEY$:IF A$="3" OR A$="4"THEN SI=VAL(A$) ELSE 115
...
175 A$ = INKEY$:IF A$="1" OR A$="2"THEN CM=VAL(A$)-1 ELSE 175
190 CLS 
...
3020 Z=1024 + 67 + (PX*3) + (PY * 32)
3021 OC = 65: IF PV=0 THEN OC=110 ELSE IF PV<0 THEN OC = 48 + ABS(PV) ELSE OC=112+PV
3030 POKE Z, OC
3040 RETURN
